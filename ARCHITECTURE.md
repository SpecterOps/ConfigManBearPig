...
(`>=0.1.3,<0.2.0`; a 0.x library makes no API-stability promise, so an uncapped floor would let a
...
- **Expected-failure triage, so severity means something.** The logs are only a debugger if `ERROR`
  implies "something broke" — and the same argument applies one level down, to `WARNING`. Four
  conditions violated that on every low-privilege run, and all are handled by classifying rather than
  by shouting:
  - **Refused registry reads.** A dozen-odd reads per site system are admin-gated. Logged one-per-read
    ...
    The summary lists the **denied key paths**, one per line, counted by *distinct key* so the number
    always matches the list — several values can be refused under one key. It names no capabilities and
    carries no remediation prose: an operator reads a registry path faster than a sentence about it, and
    README's "What a low-privilege run looks like" says the admin-gating part once. Neither this message
    nor the `SMS\Triggers` one repeats the hostname, because `LogContextFilter` prefixes
    `[target][phase]` on every record a per-host phase emits.
  - **Transport tables that were never collected.** `_sccm_expected_miss` downgrades a missing
    ...
    produced the data under the other name, or — added 2026-08-02 — no privileged table holds any
    **rows**, meaning the AdminService/WMI phases collected nothing. The second case was 106 of the 146
    WARNINGs a healthy low-privilege run emitted. The narrow condition matters: when a privileged table
    *is* populated, an absent one means that query came back empty while its transport was working, and
    stays a WARNING.
    **Rows, not existence — this is the whole subtlety.** dlt writes a resource's schema even when it
    yields zero rows, so "the table is in the catalog" and "data was collected" are different
    questions. The first implementation tested existence and was defeated by a lowpriv run into a
    *reused* output directory, whose catalog carried `adminservice_client_devices` and
    `adminservice_site_definitions` at 0 rows: 98 expected misses stayed at WARNING. `--clean` runs hid
    it, because a fresh catalog has no privileged tables at all. The same blindness affected the older
    `http_`/`smb_` branch below, which shares this helper, so fixing it can legitimately surface
    `http_`/`smb_` warnings that were previously being swallowed.
  - **Probes that come back negative.** [`_HttpProbe._request`](src/openhound_sccm/collectors/http.py)
    and [`_http_get_value`](src/openhound_sccm/collectors/privileged.py) spend most of their calls
    establishing that a host is *not* a given role. Both were WARNING. They now split by whether a
    separate conclusion line already exists: `privileged.py` logs
    `"%s is not a reachable AdminService provider; skipping"` at INFO immediately after the 404, so the
    404 itself is only evidence and drops to VERBOSE; `http.py` has no such line — it logs only
    positives between "Attempting HTTP collection on" and "HTTP collection completed" — so its connect
    failure *is* the conclusion and sits at INFO, because below that a host which served nothing would
    read as a clean collection. Status codes other than 404 are never downgraded: a 401/403 means the
    provider exists and refused you, which is a finding.
...
- **The graph output is also bundled into a zip.** After convert finishes, `run_end_to_end` calls the
  ...
  `<app.name>_collection.zip`. The zip step arrived in `openhound-collector-common` **0.1.1**
  (`graph_zip_name` / `zip_graph_output`) and **0.1.3** added `StagePaths.graph_zip`, which is how
  the summary below names the archive; the current floor of `>=0.1.3` covers both.
...
- **Consolidated output summary:** because the three phases each write their own
  ...
  merely knows the location of — so it is where their eye lands last. The path is read
  straight off `StagePaths.graph_zip`: the archive `run_end_to_end` reports having
  written, or `None` when convert emitted no `*.json` and there was nothing to bundle.
  It briefly worked the other way — the name was chosen in `collect_sccm`, passed to
  both the chain and the summary, and the summary rebuilt `graph_out / <name>` and
  re-tested existence, because `run_end_to_end` discarded `zip_graph_output`'s return
  value and `StagePaths` had no zip field. That reconstruction can disagree with what
  was actually written (a stale archive from an earlier run into the same output
  directory is the obvious case), and only one of the two is evidence. The field was
  added in `openhound-collector-common` **0.1.3**, which is why the dependency floor
  is `>=0.1.3`.
